...
SFAsset::SFAsset(SFASSETTYPE type, std::shared_ptr<SFWindow> window): type(type), sf_window(window) {
  ...
  case SFASSET_BACKGROUND:
    ...
    break;
  case SFASSET_FENCE:
    sprite =IMG_LoadTexture(sf_window->getRenderer(), "assets/Fence.png");
    break;
  }
...
void SFAsset::HandleCollision() {
  if(SFASSET_PROJECTILE == type || SFASSET_ALIEN == type || SFASSET_FENCE == type || SFASSET_COIN == type){
    SetNotAlive();
  }
}
